...
class MedianFinder:
    ...
    def addNum(self, num: int) -> None:
        ...
        while self.higher and self.higher[0] < self.lower[0]:
            heapq.heappush(self.higher, heapq.heappop_max(self.lower))
        
        while 1 < len(self.lower) - len(self.higher):
            heapq.heappush(self.higher, heapq.heappop_max(self.lower))

        while 1 < len(self.higher) - len(self.lower):
            heapq.heappush_max(self.lower, heapq.heappop(self.higher))
        
        print(f"addNum {self.lower=}, {self.higher=}")
