...
class TreeNode:
    ...
    def remove(self, key: int) -> None:
        print(f"remove {self.key=}, {key=}")
        if self.key == key:
            if not self.left and not self.right:
                print(f"flag for removal {self.key=}, {key=}")
                ...
                #print(f"remove {self.key=}, {key=}, self.right={self.right.key if self.right else None}, self.left={self.left.key if self.left else None}")
                #print(f"{successor.key=}")
        ...
        #print(f"remove {self.key=}, {key=} (AFTER) self.right={self.right.key if self.right else None}, self.left={self.left.key if self.left else None}")
...
class TreeMap:
    ...
    def getInorderKeys(self) -> List[int]:
        ret = [] if self.root is None else self.root.getInorderKeys()
